...
#convert the UNIX timestamp to the "yyyy-MM-dd" format
    ...
    nyc_df1 = nyc_df.withColumn("trip_length", col("trip_length").cast("float"))\
...
# TASK 7 - Routes Analysis
    ...
    nyc_df1_route = nyc_df1.withColumn("Route", concat_ws(" to ", col("Pickup_Zone"), col("Dropoff_Zone")))

    # Grouping by 'Route' and computing the count of total trips for UBER business
    route_profits_uber = nyc_df1_route.filter((col("business") == "Uber"))\
                                 .groupBy("Route").agg(count("*").alias("uber_count"))

    # Grouping by 'Route' and computing the count of total trips for Lyft business
    route_profits_lyft = nyc_df1_route.filter((col("business") == "Lyft"))\
                                 .groupBy("Route").agg(count("*").alias("lyft_count"))

    # Merging routes
    route_profits_merged = route_profits_uber.join(route_profits_lyft, route_profits_uber.Route == route_profits_lyft.Route)\
                                             .select(route_profits_uber["Route"], "uber_count", "lyft_count")
    # Creating total count column
    route_profits_final = route_profits_merged.withColumn("total_count", col("uber_count") + col("lyft_count"))

    # Sorting final result on Total count in descending order to find top 10 popular routes
    route_profits_final = route_profits_final.orderBy("total_count" , ascending = False)
    
    # Result
    
    route_profits_final.show(10, truncate=False)
